  ...
  try {
    ...
    const searchTool = new DynamicTool({
      ...
      func: async (searchQuery: string, runManager) => {
        const retriever = new TavilySearchAPIRetriever({
          apiKey: tavilyKey !== "" ? process.env.TAVLY_SEARCH_API : tavilyKey,
        });
    ...
    const responseSchema = z.object({
      moduleOverview: z
        .string()
        .describe(
          "A brief overview of what will be cover in this tutorial to return to the user in a paragraph form"
        ),
      learningObjectives: z.array(
        z
          .string()
          .describe(
            "The list of learning objectives of the tutorial to return to the user"
          )
      ),
      prerequisites: z.array(
        z
          .string()
          .describe(
            "The list of prerequisites that are required before starting this tutorial to return to the user"
          )
      ),
      introduction: z
        .array(
